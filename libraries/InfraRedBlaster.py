...
class InfraRedBlaster(object):
    """ Amino InfraRedBlaster Library by Frazer Smith.

    This library is designed for use with Robot Framework.

    It can be used in combination with a STB object, using STB libraries 'Set IR Blaster' keyword

    It can also be used as a standalone library (for controlling other devices such as TV's)


    TODO: Add details of how to use the library, and add help to keywords

    TODO: Add details of how to create a file for remote codes
    ...
    def load_remote(self, filename):
        self._rcu = RemoteControlConfig(filename)

    def check_valid(self, code):
        if self._rcu == None:
            raise InfraRedError("No remote control defined")
        if not code in self._rcu:
            raise InfraRedError("Invalid IR code '%s' - Check remote definition file" % code)



    def send_ir(self, port, code, power=100):
        
        self.check_valid(code)
        
        with IRNetBox(self._ir_host) as ir:
            ir.power_on()
            ir.irsend_raw(port=int(port), power=power, data=self._rcu[code])

    def send_ir_codes(self, port, *codes, **kwargs):
        ...
        waitfor = utils.timestr_to_secs(interval)

        for code in codes:
            if isinstance(code, types.ListType):
                for subcode in code:
                    self.send_ir(port, subcode, power)
                    time.sleep(waitfor)
            else:
                self.send_ir(port, code, power)
                time.sleep(waitfor)

    def set_interval(self, interval):
        """
        Set a global interval to be used by all `send ir codes` commands

        Example:
        | Set Interval  | 0.5 seconds   |
